...
const DEFAULT_VALUEBALANCE = new BitcoreBN(0);
...
export class Transaction {
    ...
    public joinSplits: Array<JSDescription>;
    public spendDescs: Array<SpendDescription>;
    public outputDescs: Array<OutputDescription>;
    public fOverwintered: boolean;
    public nVersionGroupId: number;
    public nExpiryHeight: number;

    public joinSplitPubKey;
    public branchId;
    public valueBalance;
    public joinSplitSig;
    public bindingSig;
    ...
    public toString = this.uncheckedSerialize;
    /**
     * Retrieve a hexa string that can be used with bitcoind's CLI interface
     * (decoderawtransaction, sendrawtransaction)
     *
    ...
    public toObject() {
        ...
        var obj: Partial<Transaction.txObj> = {
            hash: this.hash,
    ...
    public fromObject(arg) {
        ...
        if (this.version >= 4) {
            ...
            _.each(transaction.spendDescs, function (desc) {
                this.spendDescs.push(new SpendDescription(desc));
            });
            _.each(transaction.outputDescs, function (desc) {
                this.outputDescs.push(new OutputDescription(desc));
            });
        }
        if (this.version >= 2) {
            _.each(transaction.joinSplits, function (joinSplit) {
                this.joinSplits.push(new JSDescription(joinSplit));
            });
            if (this.joinSplits.length > 0) {
                this.joinSplitPubKey = BufferUtil.reverse(new Buffer(transaction.joinSplitPubKey, 'hex'));
     ...
     */
    public from(utxo, pubkeys, threshold) {
        if (_.isArray(utxo)) {
            var self = this;
    ...
    public _fromMultisigUtxo(utxo, pubkeys, threshold) {
    ...
    public isValidSignature(signature) {
        var self = this;
     ...
     */
    public verifySignature(sig, pubkey, nin, subscript) {
    ...
    public verify() {
        ...
        var valueoutbn = new BitcoreBN(0);
        for (var i = 0; i < this.outputs.length; i++) {
