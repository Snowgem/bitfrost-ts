...
export class ECDSA {
    ...
    public privkey2pubkey = function () {
        this.pubkey = this.privkey.toPublicKey();
    };
    public calci = function () {
        for (var i = 0; i < 4; i++) {
            this.sig.i = i;
            var Qprime;
            try {
    ...
    public deterministicK = function (badrs) {
        ...
        v = Hash.sha256hmac(v, k);
        v = Hash.sha256hmac(v, k);
        var T = BitcoreBN.fromBuffer(v);
        var N = Point.getN();
        // also explained in 3.2, we must ensure T is in the proper range (0, N)
        for (var i = 0; i < badrs || !(T.lt(N) && T.gt(BitcoreBN.Zero)); i++) {
    ...
    public toPublicKey = function () {
        ...
        var e = BitcoreBN.fromBuffer(this.hashbuf);
        var r = this.sig.r;
        var s = this.sig.s;
    ...
    public sigError = function () {
        ...
        var r = this.sig.r;
        var s = this.sig.s;
        if (!(r.gt(BitcoreBN.Zero) && r.lt(Point.getN())) || !(s.gt(BitcoreBN.Zero) && s.lt(Point.getN()))) {
            return 'r and s not in range';
        }
        var e = BitcoreBN.fromBuffer(this.hashbuf, this.endian ? {
            endian: this.endian
        } : undefined);
        var n = Point.getN();
        var sinv = s.invm(n);
    ...
    public static toLowS = function (s) {
        //enforce low s
        //see BIP 62, "low S values in signatures"
        if (s.gt(BitcoreBN.fromBuffer(new Buffer('7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0', 'hex')))) {
            s = Point.getN().sub(s);
        }
        return s;
    };
    public _findSignature = function (d, e) {
        var N = Point.getN();
        var G = Point.getG();
    ...
    public sign = function () {
        ...
        var e = BitcoreBN.fromBuffer(hashbuf, this.endian ? {
            endian: this.endian
        } : undefined);
    ...
    public static sign = function (hashbuf, privkey, endian) {
    ...
    public static verify = function (hashbuf, sig, pubkey, endian) {
        return new ECDSA().set({
            hashbuf: hashbuf,
            endian: endian,
            sig: sig,
